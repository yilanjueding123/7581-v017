...
extern INT8U   IR_flag;//红外标志位
extern void night_led_ctrl(INT8U on);//红外控制函数
	...
	//	DBG_PRINT("Handling_message_receive\r\n");
        switch (msg_id) {
        	...
        	case MSG_PERIPHERAL_TASK_USBD_DETECT_INIT:
        		usb_detect_start = 1;
				power_on_auto_rec_delay_cnt = 3*128/PERI_TIME_INTERVAL_AD_DETECT;	//2s				
        		break;
...
#if USE_ADKEY_NO
        	case MSG_PERIPHERAL_TASK_AD_DETECT_CHECK://中断
				// 按键放在外面比中断要稳定
        		ap_peripheral_key_judge();
        		...
        		#endif
				#if 0
				if(usb_state_get()==2)//火牛充电，直接进720p录像，关红外
				{
					IR_flag = 0;
					night_led_ctrl(0);
					if(huoniu_flag==0)
					{
						...
						if(huoniu_cnt > 192)
						{
							huoniu_cnt=0;
							huoniu_flag=1;
							msgQSend(ApQ, MSG_APQ_QUICK_RECORD, NULL, NULL, MSG_PRI_NORMAL);//msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
						}
					}
				}
				#endif
