...
extern void Close_tvoutput();
volatile INT8U poweron_autorecord_flag=0;
...
//        		ap_music_update_icon_status();
			...
			case MSG_APQ_MD_SET:
				...
				#if 1
				//if(motion_first_flag==1)
				//{
					if(video_720_flag == 1)
						led_type = LED_MONTION_WAIT;
					else if(video_1080_flag == 1)
						led_type =  LED_MONTION_WAIT1080;
				//}
				//motion_first_flag=0;
				#endif
				msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_LED_SET, &led_type, sizeof(INT32U), MSG_PRI_NORMAL);
        	...
        	case MSG_STORAGE_SERVICE_VID_REPLY:
				...
				if(ap_state_handling_file_creat_get())
					{
						if((ap_video_record_sts_get() & 0x04)==0x04)
						{
							if(video_720_flag == 1)
							{
					        ...
					        {
					            // bkground_del_disable(1);
								 led_type = LED_MOTION_DETECTION1080;
					             msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_LED_SET, &led_type, sizeof(INT32U), MSG_PRI_NORMAL);
					        }
						}
						else
			...
			case MSG_APQ_CONNECT_TO_PC:
				...
				//if(ap_display_get_device() != DISP_DEV_TFT) break;//如果在TVout或hdmi，则退出，TVout与pc_camera共线，所以不能这样处理，否则进不去pc_camera
				msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TV_POLLING_STOP, NULL, NULL, MSG_PRI_NORMAL);//共线，退出tvout
				Close_tvoutput();//共线，退出tvout
				OSTimeDly(10);
				#endif
				power_on_flag = 0;
...
//				audio_send_stop();
...
#endif
        		if ((ap_video_record_sts_get() & VIDEO_RECORD_BUSY) != 0) {//正在录像，关录像
			...
			*/
			...
			case MSG_APQ_TV_PLUG_IN:
        		if ((ap_video_record_sts_get() & VIDEO_RECORD_BUSY)) {//在录像退出录像
        			if(ap_video_record_func_key_active(msg_id)) {
        				break;
        			}
        		}
