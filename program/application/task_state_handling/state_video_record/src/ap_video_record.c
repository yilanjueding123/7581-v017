		...
		if(ap_state_handling_storage_id_get()!= NO_STORAGE)
            ...
            if(!cyc_restart_flag && ap_state_handling_file_creat_get())
             ...
             if((event != MSG_APQ_VDO_REC_STOP)&&(event != MSG_APQ_POWER_KEY_ACTIVE))
             	{
             	    if((ap_video_record_sts_get() & VIDEO_RECORD_MD)&&(event != MSG_APQ_TV_PLUG_IN)&&(event != MSG_APQ_TV_PLUG_OUT))
             		{
						if(video_720_flag == 1)
						{
					...
					else
					{
						if(video_720_flag == 1)
		                	...
		                	led_type = LED_WAITING_RECORD1080;
		           }
		           msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_LED_SET, &led_type, sizeof(INT32U), MSG_PRI_NORMAL);
...
extern volatile INT8U poweron_autorecord_flag;
